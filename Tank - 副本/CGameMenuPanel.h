...
class CGameMenuPanel : public CGameEntryMoveable    //继承自可移动对象
{
public:
    CGameMenuPanel();
    ~CGameMenuPanel();
    ...
    // 存放一个CGame类的指针在本中
    void SetParent(CGame* g)//设置游戏核心框架的指针
    {
        m_pParent = g;
    };
    CGame* m_pParent{ nullptr };                        //存放（一个CGame类的指针在本中）

    virtual void Draw(Graphics& gh) const;              //画自己（继承自父类
